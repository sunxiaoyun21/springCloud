...
//    }
    ...
    public Object around(ProceedingJoinPoint pjp, SystemControllerLog logAnnotation) throws Throwable {
        ...
        // String paramResult = Arrays.toString(pjp.getArgs());
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            String resultParms = JedisUtil.get(id.toString());
            JSONObject jsonObjectBefore = JSON.parseObject(resultParms);
            JSONObject jsonObjectAfter = JSON.parseObject(JSONObject.toJSONString(paramsObj));
            List<Map<String, Map>> list = CampareJsonUtil.campareJsonObjectForObj(jsonObjectBefore, jsonObjectAfter);
            list.forEach(obj -> {
