...
public class CampareJsonUtil {
    ...
    public static List campareJsonObjectForStr(String oldJsonStr, String newJsonStr1) {
        //将字符串转换为json对象
        ...
        List list = convertMapToList(differenceMap);
        return  list;
        //return jsonObject;
    }

    public static  List campareJsonObjectForObj(JSONObject oldJsonStr,JSONObject newJsonStr1){
        //递归遍历json对象所有的key-value，将其封装成path:value格式进行比较
        Map<String, Object> oldMap = new LinkedHashMap<>();
        Map<String, Object> newMap = new LinkedHashMap<>();
        convertJsonToMap(oldJsonStr, "", oldMap);
        convertJsonToMap(newJsonStr1, "", newMap);
        Map<String, Object> differenceMap = campareMap(oldMap, newMap);
        //将最终的比较结果把不相同的转换为json对象返回
        JSONObject jsonObject = convertMapToJson(differenceMap);
        List list = convertMapToList(differenceMap);
    ...
    private static List convertMapToList(Map<String, Object> map) {
    ...
    public static void main(String[] args){
        Dept dept1 = new Dept().setDeptno(1L).setDname("old").setDb_source("oldSource");
        Dept dept2 = new Dept().setDeptno(1L).setDb_source("newSource");
        String ss = JSON.toJSONString(dept1);
        String ss1 = JSON.toJSONString(dept2);
        JSONObject jsonObject = JSONObject.parseObject(ss);
        JSONObject jsonObject1 = JSONObject.parseObject(ss1);

        List<Map<String,Map>> list = campareJsonObjectForStr(ss,ss1);
        List<Map<String,Map>> list1 = campareJsonObjectForObj(jsonObject,jsonObject1);
       /* list.forEach(obj->{
            for (Iterator<Map.Entry<String, Map>> it = obj.entrySet().iterator(); it.hasNext(); ) {
                Map.Entry<String, Map> item = it.next();
                String key = item.getKey();
                Map value = item.getValue();
                System.out.println(key);
                System.out.println(value.get("oldValue") +"---->"+value.get("newValue"));
            }
        });*/
        ...
        //System.out.println(campareJsonObject(ss,ss1));
    }
}
